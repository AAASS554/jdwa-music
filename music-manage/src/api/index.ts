...
const HttpManager = {
    // 获取图片信息
    attachImageUrl: (url) => {
        if (url && url.startsWith('img/swiper/')) {
            return `http://localhost:9000/music/${url}`;
        }
    ...
    // 添加轮播图
    setBanner: (params: any) => post(`banner/add`, params),
    // 更新轮播图信息
    updateBanner: (params: any) => post(`banner/update`, params),
    // 删除轮播图
    deleteBanner: (id: string | number) => get(`banner/delete?id=${id}`),
    // 更新轮播图图片
    uploadBannerUrl: (id: string) => `${getBaseURL()}/banner/img/upload/minio${id ? '?id=' + id : ''}`
}
