...
import static com.example.jdwa.constant.Constants.SALT;
...
public class ConsumerServiceImpl extends ServiceImpl<ConsumerMapper, Consumer>
    ...
    public R updateUserAvator(MultipartFile avatorFile, int id) {
        try {
            ...
            // 上传到MinIO
            String result = minioUploadController.uploadAtorImgFile(avatorFile);
            if (!"File uploaded successfully!".equals(result)) {
                return R.error("文件上传失败: " + result);
            }
